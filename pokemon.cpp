...
public:
    ...
    Species(const std::string &_name, const int &_attackDamage, const int &_hp, const char* _tier): name(_name), attackDamage(_attackDamage), hp(_hp)
    {
    ...
    Species(const Species &source): name(source.name), attackDamage(source.attackDamage),hp(source.hp)
    {
    ...
    friend std::ostream& operator<<(std::ostream& out,const Species &other)
    {
        ...
        out<<"Name  "<<other.name<<"  Attack Damage: " << other.attackDamage<<"  hp  "<<other.hp<<" tier   "<<other.tier<<"\n";
        return out;
    };
    ...
    {
        return name == source.name && attackDamage == attackDamage && hp == source.hp && strcmp(tier,source.tier)==0;
    };
    ...
    void showInfo() override
    {
       ...
       // std::cout<<"Name  "<<name<<"  Attack Damage: " << attackDamage<<"  hp  "<<hp<<" tier   "<<tier;
...
int main()
{
    ...
    {
        std::cout<<"Pokemons are diffrient";
